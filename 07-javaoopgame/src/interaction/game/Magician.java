...
public class Magician {
	...
	// 특정 몬스터인 슬라임을 공격하는 공격 메서드
	public void huntSlame(Slame slame) {
		// 공격 전에 먼저 슬라임의 체력을 체크해서 죽은 슬라임이라면
		// 따로 공격을 할 수 없도록 메서드를 바로 탈출시킵니다.
		if(slame.getHp() <= 0) {
		...
		if(slame.getHp() <= 0) {
			...
			this.exp += 50;System.out.println("슬라임과의 교전 완료!");
			return;
		}
		// 모든 전투상황이 끝난뒤에 본인의 체력을 깎습니다.
