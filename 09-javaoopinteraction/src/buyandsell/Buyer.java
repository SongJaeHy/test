...
public class Buyer {
	...
	public void buyMango(Seller b2, int mango) {
		// 조건식 : 망고 가격보다 내 돈이 적은 경우
		if(mango * 1000 >money) {
			System.out.println("돈이 모자라 살 수 없습니다.");
			return;
		}
		// 조건식: 구매할 망고 개수보다 판매자의 재고가 적은경우
		if(b2.getMango() < mango) {
	...
	public void buyMellon(MellonSeller seller, int mellon) {
		if(mellon * 2000 <mellon) {
			System.out.println("돈이 모자라 살 수 없습니다.");
			return;
		}
		// 조건식: 구매할 망고 개수보다 판매자의 재고가 적은경우
		if(seller.getMellon() < mellon) {
			System.out.println("멜론 재고가 부족합니다.");
			return;
		}
		seller.sellmellon(mellon);
		this.money = money - (mellon * 5000);
		this.mellon += mellon;
	}
	...
	public void showBuyer() {
		...
		System.out.println("과일 개수 : " + mango);
